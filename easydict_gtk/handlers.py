...
class Handlers:
    ...
    def onLangClicked(self, button):
        ...
        self.image_language.props.file = str(
            self.cwd_images / f"flag_{self.language}.svg"
        )
    ...
    def onTrayClicked(self, status_icon, x, y, button, time, panel_position):
        if button == 1:
            if self.window.props.visible:
                ...
                self.window.show()
                if (
                    self.checkbutton_scan.get_active()
                ):  # this condition is state of check button for clipboard scan, it can be from prefdb, but this is maybe better
    ...
    def onClipboard(self, clippboard, event):
        if (
            self.window.props.visible
        ):  # first condition is check, if the window is shown
            if (
                self.checkbutton_scan.get_active()
            ):  # second condition is state of check button for clipboard scan, it can be from prefdb, but this is maybe better
                if (
                    self.clipboard.wait_for_text() != None
                ):  # if clipboard was something else, result is None, else is text, which is converted to UTF-8
                    if (
                        len(self.clipboard.wait_for_text().split()) == 1
                    ):  # this condition is check, if in clipboard is just one word
                        ...
                        )  # text from clipboard added to search entry
                        self.onSearchClicked(
                            None
                        )  # run search method which will show results in webview

    def onNonEmptyText(self, *args):
        if self.entry_search.get_text():
            self.entry_search.set_icon_from_stock(
                Gtk.EntryIconPosition.PRIMARY, "gtk-delete"
            )
        else:
    ...
    def onComboboxLanguageChanged(self, combo):
        ...
        self.image_language.props.file = str(
            self.cwd_images / f"flag_{combo.get_active_id()}.svg"
        )
